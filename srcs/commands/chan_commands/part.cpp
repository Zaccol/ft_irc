...
void	server::cmd_part(commande &param){
	...
	if (args.size() > 1){
		comment = param.get_params();
		comment.erase(comment.end() - args[1].size() - 1, comment.end());
		param.set_params(comment);
		comment = args[1];
	}
	...
	for (std::vector<std::string>::iterator it = args.begin(); it != args.end(); it++){
		...
		// get_channel_by_name(*it).send_message_to_all_users_of_this_channel(to_send);
