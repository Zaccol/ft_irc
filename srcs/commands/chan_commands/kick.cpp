...
void	server::cmd_kick(commande &param){
	...
	if (chans.size() > 1){
		for (std::vector<std::string>::iterator it = chans.begin(); it != chans.end(); it++){
			...
			to_send = ":" + list_users+"!~"+get_client_by_name(list_users).get_username()+"@"+_name+" PART "+(*it)+"\r\n";
		...
		//	declarer la classe commande avec le fd de list_users
		//	utiliser cmd_part avec cette commande 
		}
	}
	else {
		...
		for (std::vector<std::string>::iterator it = users.begin(); it != users.end(); it++){
			...
			to_send = ":" + get_client_by_name((*it)).get_nickname()+"!~"+get_client_by_name((*it)).get_username()+"@"+_name+" PART "+list_chans+"\r\n";
			...
			//	declarer la classe commande avec le fd de *it
			// ! kick ne kick pas encore !!!!!!!
			//	utiliser cmd_part avec cette commande et il faut bien le faire dans la boucle
		}
	}
