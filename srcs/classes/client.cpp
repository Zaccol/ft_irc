...
void	client::remove_all_channels_of_user(){
	// std::vector<channel>::reverse_iterator it;
//	parce que avec des iterator si on supprime le begin it n a plus de valeur comme on itere sur un truc qui change

	// for (it = _channel_list.rbegin(); it != _channel_list.rend(); it++){
	// 	cmd_part(commande((*it).get_name(), get_socket(), "PART"));
	// }
