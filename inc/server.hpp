...
class server{
	...
	private:
		...
		int							_port;
		// std::map<std::string, cmd>	_cmds_map;
	...
	public:
		...
		// std::map<int , client>		get_all_clients() const;
		int							get_max_fd();
		...
		// void						remove_one_channel_of_user(client &user, std::string which_chan);
		// void						remove_all_channels_of_user(client &user);
};
